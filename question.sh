...
#
sed -e  's/\/\([0-9]\)\([0-9]\);/-20\1\2;/g' -e 's/\/\([0-9]\)\//-0\1-/g' -e 's/^\([0-9]\)-/0\1-/g' -e 's/\//-/g' -e 's/^\([0-9][0-9]\)-\([0-9][0-9]\)-\([0-9]*\);/\3-\2-\1;/g' -e 's/,/./g' -e 's/;/,/g' -e 's/,,/,\\N,/g' -e 's/,$/,\\N/g'  -e 's/n/N/g' -e 's/,N/,\\N/g'  data.csv > output.csv